...
describe('Group lt_by_js', () => {
    ...
    it("AT_009.002 | Main menu > Marketplace verification of displayed content History bulk and it's price", () => {
        const marketplace = '#desktop-menu a[href*="marketplace"]'
        const historyBulk =
            'div:nth-child(1) > div.product-container > div:nth-child(1) > h5 > a'
    ...
    it('AT_033.004 | Header > Navigation > Verify "Marketplace" menu link', function () {
        cy.visit('https://openweathermap.org/')
        cy.get(
            '#desktop-menu a[href="https://home.openweathermap.org/marketplace"]'
        )
    ...
    it('AT_020.001 | Sign in > Dropdown menu > Verify dropdown menu options exist', () => {
        ...
        cy.get('#user-dropdown-menu li').should(($el) => {
    ...
    it('AT_009.003 | Main menu > Marketplace verification of displayed content History bulk buttons "Documentation" and "Place order"', () => {
        ...
        const documentationButton =
            'div.button-container > a[href="https://openweathermap.org/history-bulk"]'
        const placeOrderbutton =
            'div.button-container a[href="/history_bulks/new"].button-round'

        cy.visit('https://openweathermap.org/')
        cy.get(marketplace).invoke('removeAttr', 'target').click()
        cy.get(documentationButton)
            .should('be.visible')
            .invoke('removeAttr', 'target')
            .click()
