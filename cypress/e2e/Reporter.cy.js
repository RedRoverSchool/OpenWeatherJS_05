...
describe("GroupReporters", () => {
    ...
    function enterCityOrZipCode(inputText) {
        cy.get(inputSearchCity).clear().type(inputText);
        return this;
    }
    ...
    it('AT_024.002 | After clicking on "send" button, the form window automatically disappears', () => {
        ...
        cy.get(diffWeathDataSourseDropItem)
    ...
    it('AT_034.001 | <Header > verify "For Business" button', () => {
        cy.get("#desktop-menu :nth-child(10) > a")
    ...
    it("AT_001.010 | Main page > Section with search > Verify entered a city or Zip code into the Search city field", () => {
        cy.get(inputSearchCity).type("Buffalo Grove").click();
        cy.get('button[class="button-round dark"]').click();
        cy.get('ul[class="search-dropdown-menu"]').click();
        cy.url().should("include", "/city/4885955");
        cy.get('div[class="current-container mobile-padding"]').should(
            "include.text",
            "Buffalo Grove"
        );
    });
    ...
    it("AT_006.003 | Sign in > Verifying successful sign in", () => {
        ...
        cy.get(
            '#first-level-nav a[href="https://openweathermap.org/home/sign_in"]'
        ).click({ force: true });
        cy.url().should("include", "users/sign_in");
        cy.get(".input-group > #user_email").type(emailLogin);
    ...
    it("AT_002.009 | Header > Clicking the logo>Verify the logo and redirected to the Main page", () => {
        ...
        cy.get(navBarGuide).click();
        cy.url().should("include", "/guide");
        cy.get(headerGuide).should("have.text", "Guide");
        cy.get('li[class="logo"]').click();
        cy.url().should("include", "");
        cy.get(headerMainPage).should("have.text", "OpenWeather");
    });
    ...
    it('AT_024.003 | Main page > "Different weather?" option > verify 9 weather icons are shown and their text', function () {
        cy.get(differentWeatherBtn).click();
    ...
    it("AT_016.001 | Support > FAQ page > Verify Support button and FAQ link is clickable and redirects to the FAQ page", () => {
        ...
        cy.get('ul#support-dropdown-menu a[href="/faq"]')
            .should("be.visible")
            .click();
        cy.get("div.topic h1").should(
            "have.text",
    ...
    it("AT_022.007 | Footer > Social media > Verify Telegram icon redirection", () => {
        ...
        cy.get(".social a:nth-child(5)")
            .invoke("removeAttr", "target")
            .click({ force: true });
        cy.url().should("include", "t.me/openweathermap");
    });
});
